...
if (!prepareOnly) for (const directory of directories) await requireDiskSpace(directory, 8 * GiB);
...
if (!prepareOnly) await guardCommand('cargo', ['build', '--locked', '--manifest-path', path.join(core, 'Cargo.toml'), '-p', 'abra-cli',
