...
export const isPairingCode = (value: string) => /^ABRA-[A-Za-z0-9_-]{22}$/.test(value);
const digest = (purpose: string, code: string) => createHash('sha256').update(`abra-pairing-v1:${purpose}:${code}`).digest();
export const pairingCodeId = (code: string) => digest('lookup', code).toString('hex');

function ticketExpiry(ticket: string, now: number) {
  if (!ticket.startsWith('abra-pair/1/') || ticket.length > 10000) throw new Error('Invalid pairing ticket. Create a new command in Abra.');
...
export async function publishPairingTicket(ticket: string, options?: Options) {
  const { code, ...sealed } = sealPairingTicket(ticket);
  await exchange('/v1/tickets', sealed, options);
  return code;
}
