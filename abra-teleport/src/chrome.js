...
async function endpoint(port) {
  ...
  if (!/^ws:\/\/127\.0\.0\.1:/.test(body.webSocketDebuggerUrl || '')) throw new Error('Chrome exposed a non-loopback CDP endpoint');
...
export async function chromeStatus() {
  ...
  if (!state.started_at || identity.started_at !== state.started_at || !identity.command.includes(`--user-data-dir=${paths().chromeProfile}`)) return null;
  try { return { ...state, wsUrl: await endpoint(state.port) }; }
  catch { return null; }
}

export async function ensureChrome({ headless = process.platform !== 'darwin', proxy } = {}) {
  ...
  const sandboxAgent = process.platform === 'linux' && await exists(path.join(paths().home, 'agent.json'));
  ...
  const args = [
    ...
    ...(process.getuid?.() === 0 || sandboxAgent ? ['--no-sandbox'] : []),
  ...
  const state = { pid: child.pid, binary, port, profile: paths().chromeProfile, headless, proxy: requestedProxy, started_at: identity.started_at, wsUrl: await endpoint(port) };
...
export async function stopChrome() {
  ...
  const identity = await processIdentity(state.pid);
  if (!state.started_at || identity.started_at !== state.started_at || !identity.command.includes(`--user-data-dir=${paths().chromeProfile}`)) throw new Error(`refusing to stop PID ${state.pid}; profile marker does not match`);
