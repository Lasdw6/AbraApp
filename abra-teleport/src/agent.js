...
export async function agentRemote(config, argv) {
  await ensureDaemon();
...
export function agentArguments(argv, agent) {
  ...
  if (group === 'browser' && action === 'exec') {
    ...
    if (!['browser-cloud-action.js', 'browser-cloud-screenshot.js'].includes(script)) throw new Error('Only Teleport browser actions are allowed.');
