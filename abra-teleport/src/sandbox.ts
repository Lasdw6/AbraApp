...
export async function sandboxCommand(action: string, config: AgentDescriptor, payload: Record<string, any> = {}) {
  ...
  if (action === 'browser-up') {
    // Managed capture reads the live source target before creating its transfer context.
    // Closing or revoking it first would discard the tab the user selected.
    if (payload.profile !== 'active') {
  ...
  if (action === 'browser-down') {
    ...
    await browserReceive(sent.snapshot_id, { from: config.peer_id, headless: payload.headless === true });
  ...
  if (action === 'codex-down') {
    ...
    const returned = await codexReceive(sent.session_snapshot_id, { from: config.peer_id, workspace: payload.workspace || state.codex.workspace });
